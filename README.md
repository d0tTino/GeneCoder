...
- `encoded/message.fasta.manifest.json` captures encoding metrics (DNA length, bits per nucleotide, GC balance). Convert it
  into a browsable report with
  `genecli html-report --manifest encoded/message.fasta.manifest.json --output-file encoded/message_summary.html` as described in
  the [manifest guide](docs/usage.md#manifest-files).
- `simulated.manifest.json` stores the substitution, insertion, and deletion totals generated by `genecli channel run`. View
  the error-rate statistics with `jq '.metrics' simulated.manifest.json` or another JSON viewer to confirm the simulated noise
  profile before decoding.
- `decoded.txt.json` now includes a `channel` summary highlighting dropout counts, coverage histograms, and synthesis failures
  alongside the traditional substitution/insertion/deletion totals. Inspect the new fields with `jq '.metrics.channel'
  decoded.txt.json`. Multi-oligo runs also emit per-strand metrics via `jq '.metrics.oligo_metrics.coverage_counts'
  decoded.txt.json` and `jq '.metrics.oligo_metrics.dropout_flags' decoded.txt.json`, while the per-batch manifest metadata
  (including configured dropout rates and coverage distributions) lives under `jq '.metrics.sequence_batch.metadata'
  decoded.txt.json`.

For full pipeline runs the CLI also emits reusable metrics outputs:

- Every `genecli pipeline ... --output decoded.txt` invocation writes a companion `decoded.txt.json` file containing the same
  error counts, decode success rate, and ECC success metrics. The sample configuration at
  `configs/pipeline_metrics.yaml` demonstrates recording these values while the
  [`pipeline` examples](docs/pipeline_examples.md#metrics-and-troubleshooting) show additional command snippets.
- The manifest HTML report described above gives you a standalone view of each encode stage. Use it to compare multiple runs
  side by side or attach it to experiment logs.
