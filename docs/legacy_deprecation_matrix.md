...
| `genecoder.channel_sim` | `genecoder.channel_engine` + `genecoder.simulators` | Compatibility shim only | Phase 2 completion gate | Channel-equivalence tests green and runtime budget (`runtime_seconds_per_mb`) within roadmap threshold. |
| `genecoder.error_simulation.Channel` legacy path | Channel graph runtime (`genecoder.channel_engine`) | Migration in progress | Phase 3 hardening gate | Channel profile parity tests, reproducibility checks, and benchmark trend evidence attached to release decision. |
